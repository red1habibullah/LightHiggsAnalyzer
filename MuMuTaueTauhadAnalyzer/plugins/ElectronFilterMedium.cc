...
iSetup.get<SetupRecord>().get(pSetup);
  ...
  for(pat::ElectronCollection::const_iterator iele = electrons->begin() ; iele !=electrons->end(); ++iele)
    { ++Tcount;
      ...
      //cout<< "Missing intended to be int: " << GsfEleMissingHitsCut(iele) <<" _ElectronFilterMedium.cc_ "<< endl;
      //cout<<"Conversion veto cut intended to be bool:  "<< GsfEleConversionVetoCut(iele,iEvent) <<"  _ElectronFilterMedium.cc_  "<<endl;
	...
	if( (iele->full5x5_sigmaIetaIeta()<0.0106)  && (HoE < (0.046 + 1.16/E_c + 0.0324*rho/E_c)) && (abs(iele->deltaPhiSuperClusterTrackAtVtx()) <0.0547) && (GsfEleEInverseMinusPInverseCut < 0.184) && (dEtaInSeedCut < 0.0032) && (GsfEleMissingHitsCut(iele) <= 1 ) && (GsfEleConversionVetoCut(iele,iEvent)) && (iele->pt()>7))
	    {
	...
	{++EEcount;
	  //cout<<" EEloop " << endl;
	  if((iele->full5x5_sigmaIetaIeta()<0.0387) && (HoE < (0.0275 + 2.52/E_c + 0.183*rho/E_c)) && (abs(iele->deltaPhiSuperClusterTrackAtVtx()) <0.0394) && (GsfEleEInverseMinusPInverseCut < 0.0721) &&(dEtaInSeedCut <0.00632) && (GsfEleMissingHitsCut(iele) <= 1 ) && (GsfEleConversionVetoCut(iele,iEvent)) && (iele->pt()>7) )
