...
// Class:      TauEfficiencyMVA
// 
/**\class TauEfficiencyMVA TauEfficiencyMVA.cc LightHiggsAnalyzer/MuMuTaueTauhadAnalyzer/plugins/TauEfficiencyMVA.cc
...
class TauEfficiencyMVA : public edm::one::EDAnalyzer<edm::one::SharedResources>  {
   public:
      explicit TauEfficiencyMVA(const edm::ParameterSet&);
      ~TauEfficiencyMVA();
...
//
TauEfficiencyMVA::TauEfficiencyMVA(const edm::ParameterSet& iConfig):
...
TauEfficiencyMVA::~TauEfficiencyMVA()
{
...
bool TauEfficiencyMVA::isSpecificDaughter(const reco::Candidate * particle,int Id)
{
...
double TauEfficiencyMVA::RefToDaughterPhi(const reco::Candidate * particle,int Id)
{
...
double TauEfficiencyMVA::RefToDaughterEta(const reco::Candidate * particle,int Id)
{
...
std::vector<const reco::Candidate*>TauEfficiencyMVA::FindStat1Vis( const reco::Candidate * particle)
{
...
void
TauEfficiencyMVA::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
   using namespace edm;
   using namespace std;
   ...
   for (reco::PFJetCollection::const_iterator iJet = pfJets->begin(); iJet != pfJets->end(); ++iJet)
     {
	   ...
	   if(GenMatchedCombo && AnalysisCutCombo)                                                                              
		       ...
		       for(pat::ElectronCollection::const_iterator iele = electron->begin() ; iele !=electron->end() ; ++iele)
			 {
			   ...
			   PassMVACombo=(dREleTau <0.8) && (dRTauJetCombo < 0.1) &&  ((iTau->tauID(TauIdRawValue_) >-0.5) && (iTau->tauID(TauIdMVAWP_))) && ((EleFound && (dREleGenEle < 0.1)) || (EleDecay && (dREleGenDecay<0.1)));                      
...
void 
TauEfficiencyMVA::beginJob()
{
}

// ------------ method called once each job just after ending the event loop  ------------
void 
TauEfficiencyMVA::endJob() 
{
}

// ------------ method fills 'descriptions' with the allowed parameters for the module  ------------
void
TauEfficiencyMVA::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
DEFINE_FWK_MODULE(TauEfficiencyMVA);
